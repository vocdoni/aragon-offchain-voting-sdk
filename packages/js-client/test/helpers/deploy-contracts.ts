...
export async function deployvocdoniVotingContracts(
  ...
  const governanceErc20Factory = new aragonContracts.GovernanceERC20__factory();
  const governanceWrappedErc20Factory =
    new aragonContracts.GovernanceWrappedERC20__factory();
  const governanceErc20Instance = await governanceErc20Factory
    .connect(deployer)
  ...
  const vocdoniVortingPluginSetup = await vocdoniVotingFactory
    .connect(deployer)
    .deploy(
